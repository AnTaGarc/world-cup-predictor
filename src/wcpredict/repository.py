    ...
    def list_extra_time_training_rows_before(
        ...
        with self.session() as con:
            rows = con.execute(
                ...
                " WHERE pm.kickoff_utc < ? AND ps.period IN ("
                "  'extra_time_first', 'extra_time_second', 'extra_time_total'"
                " )"
                "), et AS ("
                " SELECT match_id, team_id, COALESCE("
                "  MAX(CASE WHEN period='extra_time_total' THEN xg END),"
                "  SUM(CASE WHEN period IN ('extra_time_first', 'extra_time_second') THEN xg END)"
                " ) AS extra_time_xg "
